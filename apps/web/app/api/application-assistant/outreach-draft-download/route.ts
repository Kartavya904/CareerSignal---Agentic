...
export async function GET(req: Request) {
  try {
    ...
    const draftIndexRaw = (searchParams.get('draftIndex') ?? '').trim();
    ...
    if (!Number.isFinite(draftIndex) || draftIndex < 0) {
      return NextResponse.json(
        { error: 'draftIndex must be a non-negative number' },
        { status: 400 },
      );
    }
    ...
    const drafts = (Array.isArray(contacts.drafts) ? contacts.drafts : []) as Record<
      string,
      unknown
    >[];
    ...
    if (!d) {
      return NextResponse.json(
        { error: `Draft at index ${draftIndex} not found` },
        { status: 404 },
      );
    }
    ...
    const text = platform === 'EMAIL' && subject ? `Subject: ${subject}\n\n${body}` : body;

    const safeId = analysisId.slice(0, 8);
    const safeName = contactName
      ? contactName.replace(/[^a-z0-9-_]+/gi, '-').slice(0, 32)
      : `draft-${draftIndex}`;
    const baseName = `outreach-${safeId}-${safeName}`;
    ...
    const lines = wrapTextToWidth(text, { maxWidth, font, fontSize });
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Failed to download outreach draft' },
      { status: 500 },
    );
  }
}
