...
export const users = pgTable('users', {
  ...
  /** Whether this user is included in the admin Application Analysis priority rotation. */
  applicationAnalysisPriority: boolean('application_analysis_priority').default(false).notNull(),
...
export const userPreferences = pgTable('user_preferences', {
  ...
  userId: uuid('user_id')
    ...
    .references(() => users.id, { onDelete: 'cascade' }),
  workAuthorization: workAuthorizationEnum('work_authorization').notNull(),
  workAuthorizations: jsonb('work_authorizations').$type<string[]>().default([]),
  ...
  coverLetterWordChoice: jsonb('cover_letter_word_choice').$type<string[]>().default([]),
  coverLetterNotes: text('cover_letter_notes'),
  coldLinkedinTone: jsonb('cold_linkedin_tone').$type<string[]>().default([]),
  coldLinkedinLength: varchar('cold_linkedin_length', { length: 32 }).default('SHORT'),
  coldLinkedinNotes: text('cold_linkedin_notes'),
  ...
  targetContactRoles: jsonb('target_contact_roles')
    .$type<string[]>()
    .default([
      'HIRING_MANAGER',
      'ENG_MANAGER',
