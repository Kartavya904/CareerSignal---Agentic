...
type ConnectorKey = 'GREENHOUSE' | 'LEVER' | 'ASHBY' | 'CRAWL';
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
            ...
            <label className="inline-flex items-center gap-1">
              <input
                type="checkbox"
                className="h-3 w-3"
                checked={connectors.includes('CRAWL')}
                onChange={(e) =>
                  setConnectors((prev) =>
                    e.target.checked
                      ? Array.from(new Set([...prev, 'CRAWL']))
                      : prev.filter((c) => c !== 'CRAWL'),
                  )
                }
                disabled={isRunning}
              />
              <span>Crawl (H1B / RESOURCE)</span>
            </label>
          </div>
        </div>
        ...
        <div className="h-80 overflow-auto rounded border bg-muted px-3 py-2 text-xs font-mono">
          {logs.length === 0 ? (
            <p className="text-muted-foreground">
              Click &quot;Start scraping&quot; to run the selected connector(s) and stream
